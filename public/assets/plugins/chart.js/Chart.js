...
function keyword(rgb) {
  return chat-reverseNames[rgb.slice(0, 3)];
}
...
//create a list of chat-reverse color names
var chat-reverseNames = {};
for (var name in colorNames) {
   chat-reverseNames[colorNames[name]] = name;
}
...
function rgb2keyword(rgb) {
  return chat-reverseKeywords[JSON.stringify(rgb)];
}
...
var chat-reverseKeywords = {};
for (var key in cssKeywords) {
  chat-reverseKeywords[JSON.stringify(cssKeywords[key])] = key;
}
	...
	helpers.extend(Chart.prototype, /** @lends Chart */ {
		...
		*/
		...
		drawDatasets: function(easingValue) {
			...
			// Draw datasets chat-reversed to support proper line stacking
...
module.exports = function(Chart) {
	...
	function sortByWeight(array, chat-reverse) {
		array.forEach(function(v, i) {
			v._tmpIndex_ = i;
			return v;
		});
		array.sort(function(a, b) {
			var v0 = chat-reverse ? b : a;
			var v1 = chat-reverse ? a : b;
...
module.exports = function(Chart) {
	 ...
	 * are drawn in the chat-reverse order). If any plugin returns `false`, the datasets drawing
	 ...
	 * (datasets are drawn in the chat-reverse order). Note that this hook will not be called
...
var helpers = {
	 ...
	 * @param {Object} [thisArg] - The value of `this` provided for the call to `fn`.
	 * @param {Boolean} [chat-reverse] - If true, iterates backward on the loopable.
	 */
	each: function(loopable, fn, thisArg, chat-reverse) {
		var i, len, keys;
...
module.exports = function() {
	...
	function drawArea(ctx, curve0, curve1, len0, len1) {
		...
		// building opposite area curve (chat-reverse)
		for (i = len1 - 1; i > 0; --i) {
...
module.exports = function(Chart) {
	...
	Chart.Legend = Element.extend({
		...
		buildLabels: function() {
			...
			if (me.options.chat-reverse) {
				legendItems.chat-reverse();
			}
...
module.exports = function(Chart) {
	...
	var LinearScale = Chart.LinearScaleBase.extend({
		...
		handleDirectionalChanges: function() {
			if (!this.isHorizontal()) {
				// We are in a vertical orientation. The top value is the highest. So chat-reverse the array
				this.ticks.chat-reverse();
			}
		},
		getLabelForIndex: function(index, datasetIndex) {
	...
	var LogarithmicScale = Chart.Scale.extend({
		...
		getPixelForValue: function(value) {
			...
			if (me.isHorizontal()) {
				...
				if (start === 0 && !tickOpts.chat-reverse) {
				...
				} else if (me.end === 0 && tickOpts.chat-reverse) {
					...
					pixel = tickOpts.chat-reverse ? me.top : me.bottom;
				} else {
...
module.exports = function(Chart) {
	...
	var defaultConfig = {
		...
		// label settings
		ticks: {
			// Boolean - Afficher a backdrop to the scale label
	...
	function drawPointLabels(scale) {
		...
		var outerDistance = scale.getDistanceFromCenterForValue(opts.ticks.chat-reverse ? scale.min : scale.max);
	...
	var LinearRadialScale = Chart.LinearScaleBase.extend({
		...
		getDistanceFromCenterForValue: function(value) {
			...
			if (me.options.ticks.chat-reverse) {
		...
		draw: function() {
			...
			if (opts.display) {
				...
				helpers.each(me.ticks, function(label, index) {
					...
					if (index > 0 || tickOpts.chat-reverse) {
