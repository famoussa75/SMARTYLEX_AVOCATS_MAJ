...
<div class="container-fluid">
    ...
    <!-- Title & Breadcrumbs-->
    <h2 style="text-align:center">Étape 2</h2>


    <div class="row">
        <div class="col-md-12 col-sm-12">
            <form  method="post" action="{{ route('saveJonction') }}" enctype="multipart/form-data">
            @csrf

            @foreach($arr as $value)
                <input type="hidden" name="idAudienceSource[]" value="{{ $value }}">
            @endforeach  
                    ...
                    <div class="panel-group accordion-stylist" id="accordion" role="tablist" aria-multiselectable="true">
                        ...
                        <div id="audienceFormContent" onchange="LoadSelect2Script(oSelectForm);">
                            ...
                            @if($juriductionsAud[0]->niveauProcedural=='1ère instance' && $juriductionsAud[0]->nature=='Civile')
                                @include('audiences.jonctions.jonctionForm.premiereInstanceCivile')
                            @endif
                            @if($juriductionsAud[0]->niveauProcedural=='1ère instance' && $juriductionsAud[0]->nature=='Pénale')
                                @include('audiences.jonctions.jonctionForm.premiereInstancePenale')
                            @endif
                            @if($juriductionsAud[0]->niveauProcedural=='Appel')
                                @include('audiences.jonctions.jonctionForm.appel')
                            @endif
                            @if($juriductionsAud[0]->niveauProcedural=='Cassation')
                                @include('audiences.jonctions.jonctionForm.cassation')
                            @endif
    ...
    <div class="alert alert-primary">
        <i class="fa fa-warning"></i> Toutes les pièces des audiences joints seront disponibles après la création de cette audiences <a href="#" class="alert-link"></a>.
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var totalIteration = parseInt("{{count($partiesAdverse)+count($partiesCabinet)}}");
